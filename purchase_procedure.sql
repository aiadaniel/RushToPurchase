...
CREATE PROCEDURE `rushToPurchase`.`execute_purchase`(in v_goods_id bigint, in v_phone bigint,
                                                     in v_purchase_time TIMESTAMP, out r_result int)
...
BEGIN
  ...
  ELSE
    ...
    WHERE goods_id = v_goods_id
      and end_time > v_purchase_time
      and start_time < v_purchase_time
      AND inventory > 0;
    SELECT ROW_COUNT() INTO insert_count;
    IF (insert_count <= 0) THEN
      ROLLBACK;

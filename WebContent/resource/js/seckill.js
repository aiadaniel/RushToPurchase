...
var purchase = {
    ...
    detail: {
        ...
        init: function (params) {
            ...
            //验证手机号
            if (!purchase.validatePhone(killPhone)) {
                //绑定手机 控制输出
            ...
            $.get(purchase.URL.now(), {}, function (result) {
                if (result && result['success']) {
                    var nowTime = result['data'];
                    //时间判断 计时交互
                    purchase.countDown(goodsId, nowTime, startTime, endTime);
                } else {
    ...
    countDown: function (goodsId, nowTime, startTime, endTime) {
        ...
        if (nowTime > endTime) {
            ...
            seckillBox.countdown(killTime, function (event) {
                ...
                purchase.handlerSeckill(goodsId, seckillBox);
            });
        } else {
            //秒杀开始
        	purchase.handlerSeckill(goodsId, seckillBox);
    ...
    handlerSeckill: function (goodsId, node) {
        //获取秒杀地址,控制显示器,执行秒杀
        node.hide().html('<button class="btn btn-primary btn-lg" id="killBtn">开始秒杀</button>');

        $.post(purchase.URL.exposer(goodsId), {}, function (result) {
            //在回调函数种执行交互流程
            if (result && result['success']) {
                ...
                if (exposer['exposed']) {
                    ...
                    //获取秒杀地址
                    var md5 = exposer['md5'];
                    var killUrl = purchase.URL.execution(goodsId, md5);
                    ...
                    var end = exposer['end'];
                    purchase.countDown(goodsId, now, start, end);
                }
            } else {
                console.log('result: ' + result);
            }
